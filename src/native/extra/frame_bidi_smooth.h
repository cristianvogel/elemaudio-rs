...
namespace elem
{
    ...
    struct FrameBiDiSmoothNode : public GraphNode<FloatType> {
        using GraphNode<FloatType>::GraphNode;
        using Sample = FloatType;

        static constexpr size_t CHILD_ATTACK_TIME = 0;
        static constexpr size_t CHILD_RELEASE_TIME = 1;
        static constexpr size_t CHILD_ATTACK_SHAPER = 2;
        static constexpr size_t CHILD_RELEASE_SHAPER = 3;
        static constexpr size_t CHILD_INPUT = 4;
        static constexpr size_t NUM_CHILDREN = 5;

        FrameBiDiSmoothNode(NodeId id, double sr, int blockSize)
        ...
        void process(BlockContext<FloatType> const& ctx) override
        {
            ...
            auto const* attackTimeSignal = ctx.inputData[CHILD_ATTACK_TIME];
            auto const* releaseTimeSignal = ctx.inputData[CHILD_RELEASE_TIME];
            auto const* attackShaperSignal = ctx.inputData[CHILD_ATTACK_SHAPER];
            auto const* releaseShaperSignal = ctx.inputData[CHILD_RELEASE_SHAPER];
            ...
            for (size_t i = 0; i < numSamples; ++i) {
                ...
                auto const input = inputSignal[i];
                auto const rising = input >= currentValues[track];

                auto const baseTime = rising ? std::max(Sample(0), attackTimeSignal[i]) : std::max(Sample(0), releaseTimeSignal[i]);
    ...
    private:
        ...
        static int32_t durationFrames(Sample timeSeconds, Sample frameDurationSeconds)
        {
            if (timeSeconds <= Sample(0) || frameDurationSeconds <= Sample(0)) {
                return 1;
            }

            auto const frames = static_cast<int32_t>(std::ceil(timeSeconds / frameDurationSeconds));
